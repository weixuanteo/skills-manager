import { Layers, Monitor, Moon, RefreshCw, Search, Sun, X, ArrowUpCircle } from 'lucide-react'
...
export function Header({ query, setQuery, onRescan, scanning, onCheckUpdates, checking, theme, setTheme, total, updatesAvailable, inputRef }: Props) {
  ...
  return (
    <header className="h-14 shrink-0 border-b border-[var(--border)] bg-[var(--bg-elev)]/80 backdrop-blur flex items-center gap-3 px-4">
      ...
      <div className="flex items-center gap-2">
        <button type="button" className="btn" onClick={onCheckUpdates} disabled={checking || scanning} title="Check every skill for updates">
          ...
          <span className="hidden md:inline">{checking ? 'Checking…' : 'Check updates'}</span>
        </button>
