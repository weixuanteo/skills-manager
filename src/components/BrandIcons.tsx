...
/** OpenAI mark, used for Codex. */
export function CodexIcon({ className = 'h-3.5 w-3.5', ...rest }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className={className} {...rest}>
...
export function ClaudePluginIcon({ className = 'h-3.5 w-3.5' }: { className?: string }) {
  return (
    <span className={`relative inline-flex shrink-0 ${className}`} aria-hidden="true">
      ...
      <span className="absolute -right-[3px] -bottom-[3px] inline-flex items-center justify-center rounded-full bg-[var(--bg)] p-[1.5px]">
        <Puzzle className="h-[7px] w-[7px]" strokeWidth={3} />
      </span>
    </span>
  )
}

const BRAND_ICONS: Partial<Record<AgentId, (props: { className?: string }) => ReactElement>> = {
  claude: ClaudeIcon,
  codex: CodexIcon,
